...
export function Tamalesdetalle() {
  ...
  const obtenderData = () => {
    axios
      .get(`${url2}/${id}`)
      .then((response) => setTamales(response.data))
  ...
  return (
    <div>
      <Detalleimg src={tamales.imagen} alt="tamal"></Detalleimg>
      <div>
        <Descripimg>
          <Detailnombre>{tamales.nombre}</Detailnombre>
          <Detailprecio>{tamales.precio}</Detailprecio>
        </Descripimg>
      </div>
      ...
      <Buttondiv>
        <Buttoncarritoagregar onClick={() => {
          ...
          Agregar {counter} al carrito {counter * tamales.precio}
        </Buttoncarritoagregar>
      </Buttondiv>
    </div>
  );
